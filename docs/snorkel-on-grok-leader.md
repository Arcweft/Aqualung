...
- `session/prompt` 已经在本机 `launch-home` 上跑通：host `GROK_HOME` 登录，独立的 `leader.sock`（不是交互窗口那条）。证明是 `session/prompt` 的 `stopReason`。
- 默认 `$HOME/.grok/leader.sock` 上也跑通了：常驻 `grok agent leader --no-exit-on-disconnect`，`launch-home --socket` 接上去，手机 `session/prompt` 得到 `stopReason`。同一颗 socket 上再开 `grok agent --leader stdio` 做 `session/load`，历史回放带 `isReplay` 和 `x.ai/leaderClientId`，内容是手机刚打的那一轮。这是两路 follower 共用一份 Agent。交互 TUI 当时没开着；`[cli] use_leader = true` 之后，再开 `grok` 会作为第三路 follower 连同一颗 socket。`session/new` 的 cwd 是 `/tmp`，别的目录里 `grok sessions list` 看不见这场，要用 `sessionId` resume。
