...
Any safety fact you can't get to step 4, say so out loud. Don't write it up as settled. Step 4 is usually one small test or script that calls the same code the app ships.

In this repository, step 5 for snorkel or topside is `verify-aqualung` (doctor, then the matching feature path on 1943 or 7678). `cargo test` is step 4, not step 5, unless the test is the user path.
...
3. Look where grep stops. Read the source of the library you call, and check its pinned version and any local patch. Work out when things run: a dropped snorkel versus a closed 7678, a missing unix socket versus a live home agent, one JSON-RPC object per WebSocket frame versus concatenated frames. Follow what a symbol search misses: the JSON an API returns, a wire format, another language reading the same bytes, a feature flag, code three hops downstream.
...
5. Prove the one fact. Write a test or script that runs the real code, run it, and paste what happened. If you can't prove it cheaply, mark it unproven. Don't round up.
6. For a big or wide change, run several independent reviews of the same question and merge what they catch. If the host cannot do that, do one thorough pass and say so.
