...
At the end of the run, before handing back, check the log told the truth. Walk the log against what actually happened this session (and against the host's chat log if one exists; if it does not, skip that check and say so):
...
Before handing back, get a second look at the trail. If the host can spawn a worker, do so. If it cannot, the parent self-checks and says so. The reviewer reads the audit trail (and the session log if one exists), then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
...
Every reply for a run that produced a trail ends with an "Attention" section. Lead with whether this was an independent worker or a parent self-check, then list each flag pointing to specific rows or moments. "No flags" is a valid value.
