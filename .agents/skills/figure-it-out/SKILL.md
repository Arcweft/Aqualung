...
When the task is too large or too open for a single pass, design a workflow first. The deliverable before any code is the workflow itself: a sequence of phases that scales rigor to the task, runs the scientific method, and leaves a decision trail a human can audit after stepping away. Bias toward more rigor. The cost of building the wrong thing dwarfs the cost of being careful.

Don't reinvent a skill you already have. A focused walkthrough is `how`. Motivation is `why`. A small diff you don't trust is `blast-radius`. Types and module shape before code is `architect`. N candidates is `arena`. Coverage fan-out is `swarm`. Adversarial review is `interrogate`. This skill is for a large or cross-cutting effort (a migration across many call sites, an ambitious multi-part change), or work the user reviews after stepping away. The rigor and the audit trail are the point.

Workers follow [host.md](../host.md). Do not name a model.

## Start

Open a todolist whose first item is to read the principle skills that apply to this run, at least **prove-it-works**, **sequence-verifiable-units**, **foundational-thinking**, and **never-block-on-the-human**. Then add the phases below as todos.
...
- Build the verification harness before the work, with the baseline captured from the pre-change state, so the check reads as "old value vs new value". In this repository that harness is `verify-aqualung` plus `cargo test` where a crate test is the right grain.
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with diverse, isolated, opinionated candidates. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
