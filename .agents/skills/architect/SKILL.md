...
Design it twice. Require at least two structurally distinct candidates before synthesis, even when the first looks sufficient. This is the **exhaust-the-design-space** principle skill made concrete. Whole-shape alternatives, not point fixes inside one shape. If the host cannot spawn workers, run the two candidates yourself in sequence, each in its own output directory.
...
- Types that need escape hatches (`any`, casts, `unwrap` on a path that should be closed, optional fields always set in practice) to compile.
