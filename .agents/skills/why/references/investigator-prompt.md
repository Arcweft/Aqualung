...
Build each source search from this template. Fill in the placeholders. For git and `gh`, append `sources/code-archaeology.md`. If the target code looks defensive (null checks, retry logic, timeout handling, rate limiting, feature flags, protocol guards), also append `sources/incident-postmortem.md`.

---

You are investigating the historical context and motivation behind a piece of code. A later pass combines your findings with other sources into a final answer, so gather evidence accurately rather than writing prose.

If other searches are running in parallel, don't try to cover everything. Focus on your assigned source and go deep.
...
3. **Follow links within your assigned source.** If a PR references another PR or commit, pull it. Stay inside your assigned source. When you spot a cross-source reference, do NOT chase it yourself. Record it under "Additional Leads".
...
Don't synthesize or form a final opinion on "the why." Collect the raw material honestly and completely.
...
### Source
Which source you investigated (source control, issues, docs, chat, observability, errors, analytics, in-repo comments).
