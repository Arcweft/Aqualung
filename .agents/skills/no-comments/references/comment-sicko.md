...
- `#[rustfmt::skip]`. Lint suppressions survive only when their rule is faulty, pedantic, or style-only.
...
`#[allow(...)]`, `clippy::`, `allow(dead_code)`, `allow(unused)`, and similar suppressions stink. Look up the lint. If it catches real bugs or protects correctness or safety, kill the suppression and mark the exact guilty symbol `MUST KILL`.
